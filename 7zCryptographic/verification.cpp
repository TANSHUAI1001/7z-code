...
//    AesGenTables(); //step 0
     ...
     * local
     */
//    Aes_SetKey_Dec(_aes + offset + 4,key,keyLen);  // step 1
    uint8 *iv_arr = pCd->iv;
    printf("iv:\n");
    for(i = 0; i < 16; i++){
//        printf("%.2x ",iv_arr[i]);
...
//    printf("\n\n");
//
//    AesCbc_Init(_aes + offset,(uint8 *)iv_arr); //step 2
//    printf("_aes:\n");
//    for(i = 0; i < _aesLen; i++){
//        printf("%.2x ",_aes[i]);
//    }
...
////    uint8 *cipher = new uint8[cipherLen];
////    strcpy((char *)cipher,pCd->cipher.c_str());
...
//        cout<<hex<<int(cipher[i])<<" ";
    ...
    */
    ...
    margin = nbytes = cipherLen - pCd->unpackSize;
    index = cipherLen - 1;
    while(nbytes > 0){
        if(out[index] != 0){
            cout<<"-1"<<endl;
...
//    AesCbc_Decode(_aes + offset,(uint8 *)cipher,1); // step 3

//    printf("cipher:\n");
//    for(i = 0; i <pCd->cipherLen; i++){
//        printf("%.2x ",cipher[i]);
//    }
//    printf("\n\n");
//    CRC32_t crc;
...
//    unsigned int crcc;
//    CRC32_Init(&crc);
////    CRC32_Update(&crc,cipher,cipherLen);
//    CRC32_Update(&crc,cipher,pCd->unpackSize);
////    CRC32_Update(&crc,cipher,pCd->unpackSize);
//    CRC32_Final(crc_out,crc);
//    cout<<"pCd->unpackSize "<<pCd->unpackSize<<endl;
